...
abstract class Repository {
  ...
  /// [toEncodable] converts T to JSON-serializable form for storage (e.g. model.toJson)
  Future<ApiResponse<T>> executeWithCache<T>({
    required Future<ApiResponse<T>> Function() remoteCall,
    String? cacheKey,
    Duration? cacheDuration,
    T Function(dynamic)? parser,
    dynamic Function(T)? toEncodable,
  }) async {
    ...
    if (response.success && 
        ...
        cacheKey != null) {
      final dataToStore = toEncodable != null ? toEncodable(response.data as T) : response.data;
      await _cacheData(cacheKey, dataToStore);
    }
